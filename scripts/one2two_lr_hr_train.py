#!/usr/bin/python3
# the trained lr images are bad, 
# this may because _load_lr_mag return the -1 to 1
# data of complex and real part and then we (*2) - 1 to this data
...
CUDA_VISIBLE_DEVICES=7 python3 /home/data1/musong/workspace/python/spen-recons/scripts/one2two_lr_hr_train.py \
--dataroot /home/data1/musong/workspace/python/spen-recons/data/IXI_sim \
--log_dir /home/data1/musong/workspace/python/spen-recons/log/one2two_IXI_sim
"""
...
def _load_lr_mag(path: str) -> torch.Tensor:
    ...
    real = np.real(arr)
    imag = np.imag(arr)
    out = np.stack([real, imag], axis=0)  # 2 x H x W
...
parser.add_argument('--log_dir', type=str, default='/home/data1/musong/workspace/python/spen-recons/log/one2two_IXI_sim', help='directory to save logs and model checkpoints')
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        # Total loss
        loss_G = loss_GAN_hr2lr + loss_GAN_lr2hr + loss_cycle_hrlrhr + loss_cycle_lrhrlr
        loss_G.backward()
        ...
        logger.log({'loss_G': loss_G, 'loss_G_GAN': (loss_GAN_hr2lr + loss_GAN_lr2hr),

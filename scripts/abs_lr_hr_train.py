...
CUDA_VISIBLE_DEVICES=7 python3 /home/data1/musong/workspace/2025/8/08-20/tr/scripts/abs_lr_hr_train.py \
--dataroot /home/data1/musong/workspace/2025/8/08-20/tr/data/IXI_sim \
--cuda \
--log_dir /home/data1/musong/workspace/2025/8/08-20/tr/log/abs_IXI_sim
"""
...
class Logger:
    def __init__(self, n_epochs, batches_epoch, log_dir='logs', log_freq=100, running_avg=False):
    ...
    def log(self, losses=None, images=None):
        ...
        # Save images every log_freq
        def tensor2image(tensor):
            t = tensor.detach()
            img = 127.5 * (t[0].cpu().float().numpy() + 1.0)
            if img.shape[0] == 1:
                img = np.tile(img, (3, 1, 1))
            img = np.transpose(img, (1, 2, 0))
            return img.astype(np.uint8)

        if (self.batch % self.log_freq) == 0 and images:
...
class ImageDataset(Dataset):
    ...
    def __init__(
        ...
        # Collect .mat files
        self.file_lr = sorted(glob.glob(os.path.join(root, "hr", "*.mat")))
        self.file_hr = sorted(glob.glob(os.path.join(root, "lr", "*.mat")))
    ...
    def _load_lr_mag(self, path: str) -> torch.Tensor:
    ...
    def __getitem__(self, index: int):
        path_hr = self.file_lr[index % len(self.file_lr)]
        if self.unaligned:
            j = random.randint(0, len(self.file_hr) - 1)
        else:
            j = index % len(self.file_hr)
        path_lr = self.file_hr[j]

        hr = self._load_hr(path_hr) * 2 - 1       # HR (real) -> 3ch tensor
        lr = self._load_lr_mag(path_lr) * 2 - 1   # LR (complex) -> |.| -> 3ch tensor
...
parser.add_argument('--log_dir', type=str, default='/home/data1/musong/workspace/2025/8/08-20/tr/log/abs_IXI_sim', help='directory to save logs and model checkpoints')
parser.add_argument('--ckpt_save_freq', type=int, default=50, help='save checkpoint frequency (in epochs)')
parser.add_argument('--log_freq', type=int, default=20, help='logger frequency')
...
# Networks
netG_hr2lr = Generator(opt.input_nc, opt.output_nc)
netG_lr2hr = Generator(opt.output_nc, opt.input_nc)
netD_hr = Discriminator(opt.input_nc)
netD_lr = Discriminator(opt.output_nc)
...
netG_hr2lr.apply(weights_init_normal)
...
optimizer_G = torch.optim.Adam(itertools.chain(netG_hr2lr.parameters(), netG_lr2hr.parameters()),
...
# Loss plot
logger = Logger(opt.n_epochs, len(dataloader), f'{opt.log_dir}/train', opt.log_freq)
###################################
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        same_lr = netG_hr2lr(real_lr)
        loss_identity_lr = criterion_identity(same_lr, real_lr)*5.0
        # G_B2A(A) should equal A if real A is fed
        same_hr = netG_lr2hr(real_hr)
        loss_identity_hr = criterion_identity(same_hr, real_hr)*5.0
        ...
        loss_GAN_lr2hr = criterion_GAN(pred_fake, target_real)

        # Cycle loss
        recovered_hr = netG_lr2hr(fake_lr)
        loss_cycle_hrlrhr = criterion_cycle(recovered_hr, real_hr)*10.0

        recovered_lr = netG_hr2lr(fake_hr)
        loss_cycle_lrhrlr = criterion_cycle(recovered_lr, real_lr)*10.0

        # Total loss
        loss_G = loss_identity_hr + loss_identity_lr + loss_GAN_hr2lr + loss_GAN_lr2hr + loss_cycle_hrlrhr + loss_cycle_lrhrlr
        loss_G.backward()
        ...
        # Total loss
        loss_D_lr = (loss_D_real + loss_D_fake)*0.5
        loss_D_lr.backward()
        ...
        logger.log({'loss_G': loss_G, 'loss_G_identity': (loss_identity_hr + loss_identity_lr), 'loss_G_GAN': (loss_GAN_hr2lr + loss_GAN_lr2hr),
                    'loss_G_cycle': (loss_cycle_hrlrhr + loss_cycle_lrhrlr), 'loss_D': (loss_D_A + loss_D_lr)}, 
                    images={'real_hr': real_hr, 'real_lr': real_lr, 'fake_hr': fake_hr, 'fake_lr': fake_lr})
    ...
    if (epoch % opt.ckpt_save_freq) == 0:
        ...
        torch.save(netG_hr2lr.state_dict(), f'{opt.log_dir}/weights/netG_hr2lr.pth')
        ...
        torch.save(netD_lr.state_dict(), f'{opt.log_dir}/weights/netD_lr.pth')
